...
double BoxMuller(ifstream &uni_num, double mu, double sigma)
{
  ...
  //  }
  // while ( u1 <= epsilon );
  uni_num.read((char*)&uni_i, sizeof(uni_i)); 
  u1 = uni_i / 65536.0 / 65536.0;
  uni_num.read((char*)&uni_i, sizeof(uni_i)); 
  u2 = uni_i / 65536.0 / 65536.0;
...
float r4_nor_file_input ( ifstream &uni_num, uint32_t kn[128], float fn[128], float wn[128] )
...
//
{
  ...
  uint32_t uni_i;
  float uni;
  const int hz_size = sizeof(hz);
  const int uni_i_size = sizeof(uni_i);
  ...
  else
  {
    for ( ; ; )
    {
      if ( iz == 0 )
      {
        for ( ; ; )
        {
          uni_num.read((char*)&uni_i, uni_i_size);
          uni = uni_i / 65536.0 / 65536.0;
          x = - 0.2904764 * log ( uni );
		  uni_num.read((char*)&uni_i, uni_i_size);
          uni = uni_i / 65536.0 / 65536.0;
          y = - log ( uni );         
